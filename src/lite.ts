...
import { Duplex, type DuplexEvents, type DuplexOptions } from 'streamx'
import { type Hex } from 'uint8-util'
...
export type SignalEvent = {
  type?:'renegotiate'|'candidate'| 'transceiverRequest'|RTCSdpType;
  renegotiate?:boolean;
  candidate?:{
...
export type PeerOpts = DuplexOptions<any> & {
    initiator:boolean;
    constraints:{
...
export interface PeerEvents {
    'signal':(data:SignalEvent)=>void;
    'error':(err:Error)=>void;
    '_iceComplete':()=>void;
    'connect':()=>void;
    'iceTimeout':()=>void;
    'iceStateChange':(iceConnectionState, iceGatheringState)=>void;
    'signalingStateChange':(data:RTCSignalingState)=>any;
    'negotiated':()=>any
    'stream':(ev:MediaStream)=>any|void;
    'track':(ev:MediaStreamTrack, s:MediaStream)=>any|void;
}
...
class Peer extends Duplex<
    ...
    true,
    true,
    (PeerEvents & DuplexEvents<any, any>)
> {
    static WEBRTC_SUPPORT:boolean = !!RTCPeerConnection
    ...
    allowHalfTrickle
    iceCompleteTimeout
    protected _isReactNativeWebrtc
    ...
    localFamily?:'IPv6'|'IPv4'
    localPort?:number
    protected _pcReady = false
    protected _channelReady = false
    protected _pc?:RTCPeerConnection|null
    protected _iceComplete = false // ice candidate trickle done (got null candidate)

    // send an offer/answer anyway after some timeout
    protected _iceCompleteTimer:null|NodeJS.Timeout = null
    protected _channel:RTCDataChannel|null = null
    protected _pendingCandidates:any[] = []
    protected _senderMap?:Map<MediaStreamTrack, any>|null
    protected _isNegotiating = false // is this peer waiting for negotiation to complete?
    protected _firstNegotiation = true
    protected _batchedNegotiation = false // batch synchronous negotiations
    protected _queuedNegotiation = false // is there a queued negotiation request?
    protected _sendersAwaitingStable:any[] = []
    ...
    // the funny thing is that if you uncomment this it breaks everything
    // even just setting it to `undefined` breaks it
    ...
    signal (data:Partial<RTCSessionDescriptionInit & {
        renegotiate?:boolean
        transceiverRequest
        candidate
    }>) {
        ...
        if (
            !data.sdp &&
            !data.candidate &&
            !data.renegotiate && !data.transceiverRequest
        ) {
    ...
    _setupData (event) {
        ...
        this._channel.onerror = event => {
            const err = event.error instanceof Error ?
                event.error :
                // @ts-expect-error ???
                new Error(`Datachannel error: ${event.message}` +
                    // @ts-expect-error ???
    ...
    // statreports can come with a value array instead of properties
        ...
        // Unknown browser, skip getStats() since it's anyone's
        // guess which style of getStats() they implement.
        } else {
            cb(null, [])
        }
    }
    ...
    _maybeReady () {
        ...
        // HACK: We can't rely on order here, for details
        // see https://github.com/js-platform/node-webrtc/issues/339
        const findCandidatePair = () => {
            ...
            ((item.type === 'candidatepair' || item.type === 'candidate-pair') && item.selected)
                ...
                // are unsupported, fallback to using setInterval to
